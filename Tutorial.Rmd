...
<center>[Tutorial on tidymodels for Machine Learning.](https://hansjoerg.me/2020/02/09/tidymodels-for-machine-learning/#updates) </center>
...
{caret}은 데이터 전처리부터 교차검증까지의 모든 과정을 수행하는 R 유저들이 흔히 쓰는 머신러닝 패키지이다. {tidymodels}에 관한 글에서 {caret} 얘기를 먼저 꺼낸 이유는 {tidymodels}이 {caret}을 거의 완벽하게 대체할 수 있기 때문이다. 무엇보다도, {tidyverse}에 익숙한 유저들이라면 {tidymodels}을 이용하면 {caret}보다 훨씬 더 편하고 직관적(user-friendly)인 모델링을 수행할 수 있다. 이제 {tidymodels}의 늪으로 빠져보자!`r emo::ji("runner")`
...
```{r}
library(conflicted) # for conflict_prefer()
library(tidymodels)
...
`conflict_prefer("filter", "dplyr")`는 {dplyr}의 `filter()` 함수를 최우선으로 하기 위함이다.

### 1.2 데이터 불러오기
본 튜토리얼에서 이용할 toy data`r emo::ji("video_game")`는 `diamonds{ggplot2}``r emo::ji("gem")`이다. 해당 데이터는 다이아몬드의 등급과 크기 및 가격에 관한 정보를 갖는다.
...
{broom} 패키지를 구성하는 첫 번째 함수로 `glance()`를 소개한다. glance라는 동사가 우리나라 말로 힐끗 본다는 말을 갖는다는 점에서 추측할 수 있듯이, `glance()`는 적합된 모형의 전체적인 정보를 간략히 제공해준다. 이런 것을 보면 함수 이름을 지을 때 개발자가 얼마나 심사숙고했는지를 알 수 있고, R에서 함수 이름을 신중하게 정하는 것은 결코 시간을 낭비하는 행위가 아니라 코드의 가독성을 높여줄 수 있는 좋은 습관임을 한 번 더 깨달을 수 있다. 참 애정이 가는 패키지다.`r emo::ji("yum")`
```{r, cache = TRUE}
knitr::kable(glance(lm_fit1$fit), "simple")
```

`knitr::kable()`은 HTML에서 출력을 이쁘게 만들어주기 위한 함수이므로 무시해도 된다. 적합된 모형의 수정된 $R^2$ 값은 98.2%로 상당히 설명력을 자랑한다. 또한, RMSE는 해당 결과의 sigma를 통해 확인할 수 있으며, 0.132 값을 갖는다. 다음으로 `tidy()`는 추정된 모수에 대한 정보를 제공한다. 다음의 결과에서 우리는 carat의 2차 효과가 유의하게 존재함을 알 수 있다. 통계량의 크기를 기준으로 내림차순으로 정렬하여 표시하였다.

```{r, cache = TRUE}
tidy(lm_fit1) %>% 
    arrange(desc(abs(statistic)))
```
마지막으로 `augment()`를 통해 모형의 예측값, 적합값 등을 반환해준다. augment는 우리나라 말로 어떤 것의 양 또는 값, 크기 등을 늘리는 것(to increase the amount, value, size, etc. of something)을 뜻하는 동사로, 해당 함수도 이름을 통해 어느정도 그 역할을 가늠할 수 있다.
...
원자료의 각 행을 의미하는 관측값(observed values)이라는 표현과 실제값(actual values)라는 말은 통용되니 어떤 용어를 써도 문제가 없다. 특히, 머신러닝에서는 이를 데이터 포인트(data point)라고 표현하기도 한다. 3가지 용어 모두 통용되는 말이니 몰랐다면 알아두자. 모든 학문에서 그렇겠지만 통계학에서는 특히 정확한 용어 정의가 중요하므로, 비슷한 용어 또는 비슷한 듯 다른 용어들이 있다면 틈틈이 정리하는 습관을 꼭 갖도록 하자.
...
즉, 아직 랜덤포레스트 모형의 적합에 관장하는 모수 값을 결정하지 않았으므로 모형을 훈련 자료에 적합할 준비가 된 상태가 아니라고 할 수 있다. 그리고, `mtry`의 최댓값은 `update()`를 사용해 원하는 값을 명시할 수도 있고, 또는 `finalize()`를 사용해 해당 자료가 갖는 예측변수의 수로 지정할 수도 있다.
```{r}
rf_model %>% 
  parameters() %>% 
...
이 글의 시작에서 {tidymodels}이 {caret}을 거의 완벽하게 대체한다고 언급하였는데, 100%가 아닌 이유는 다른 여러 머신러닝 모형들을 결합해주는(i.e., ensemble, stacking, super learner) 기능을 갖는 패키지가 아직 없기 때문이다. {caret}에는 이러한 기능을 수행하는 [caretEnsemble](https://cran.r-project.org/web/packages/caretEnsemble/index.html)이 존재하며, RStudio의 {tidymodels}을 개발하는 팀에서 이와 비슷하게 작동하는 패키지를 언젠간 출시할 것으로 생각한다. 이것을 기다리기 힘든 사람들은 [Alex Hayes의 블로그](https://www.alexpghayes.com/blog/implementing-the-super-learner-with-tidymodels/)를 참고해보길 바란다. 물론, 이 튜토리얼만 step by step으로 따라 하고 잘 이해한다면, 실무에서 머신러닝이 필요한 경우에도 전혀 무리가 없을 것으로 생각한다.

이 글은 {tidymodels}을 소개하는 질 좋은 튜토리얼 자료를 찾고 있는 한국 R 유저들을 위해 작성하였다. 아무래도 {tidymodels}가 나온 지 오래된 패키지는 아니다 보니, 영어로 작성된 훌륭한 튜토리얼은 많았으나 한국어로 된 질 좋은 자료는 찾기 힘들었기 때문이다. 비록 해당 튜토리얼이 다른 글을 기반으로 참고하여 작성된 것이긴 하지만`r emo::ji("sweat")`, {tidymodels}을 배우길 원하는 한국 R 유저들에게 조금이나마 도움이 됐으면 좋겠다.`r emo::ji("blush")` 마지막으로 이 튜토리얼을 진행하는 데 쓰인 컴퓨터 session에 대한 정보를 제공하는 것을 끝으로, 이 글을 마무리한다.
